    ...
    fn delete(&mut self, val: i32) -> bool {
        ...
        while current.is_some() {
            if current.as_ref().unwrap().value == val {
                // Take full ownership of the target node out of 'crruent'
                let mut target_node = current.take().unwrap();
                // Re-link '*current' to bypass the target node
                *current = target_node.next.take();
                return true;
            }
            // Advanced to next pointer
...
// Optional display trait for pretty printing
