...
const SECRET_KEY: &[u8] = b"supersecrekeyyoushouldchangeforsure";
    ...
    if let Some(header_value) = auth_header {
        if let Some(token) = header_value.strip_prefix("Bearer ") {
            ...
            return match token_data {
                Ok(data) => HttpResponse::Ok().body(format!("🔓 Welcome, {}!", data.claims.sub)),                Err(_) => HttpResponse::Unauthorized().body("❌ Invalid token"),
            };
        }
    }
